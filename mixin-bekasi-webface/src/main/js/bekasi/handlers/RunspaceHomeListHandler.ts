...
export default class RunspaceHomeListHandler extends BaseHandler {

    public handle(request: RunspaceHomeListRequest, callback: (list: RunspaceItemList) => void): void {
        let director = directors.getRunspaceDirector(this.controller);
        director.getHomeList(callback);
    }
