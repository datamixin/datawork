...
export default class TopLevelFacetSpecDesignView extends TopLevelSpecDesignView {
	...
	public createControl(parent: Composite, index: number): void {
		...
		element.addClass("vegazoo-top-level-facet-spec-design-view");
		element.css("border", TopLevelSpecDesignView.BORDER_WIDTH + "px solid transparent");

		let layout = new GridLayout(1, TopLevelFacetSpecDesignView.MARGIN, TopLevelFacetSpecDesignView.MARGIN, 0, 0);
		this.composite.setLayout(layout);

		this.composite.onSelection(() => {
			let request = new TopLevelFacetSpecSelectionRequest();
	...
	public adjustHeight(): number {
		...
		let border = TopLevelFacetSpecDesignView.BORDER_WIDTH * 2;
		let adjuster = new GridCompositeAdjuster(this.container);
		height += adjuster.adjustHeight();
		return height + TopLevelFacetSpecDesignView.MARGIN * 2 + border;
	}
