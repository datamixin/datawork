...
export default class TimeUnitCustomView extends ValueDefCustomView {
    ...
    private createValuePanel(panel: CustomCompositePanel): void {
        ...
        this.valuePanel.setInputCallback((callback: (list: string[]) => void) => {
            let values: string[] = [];
            let keys = Object.keys(TimeUnit);
            for (let key of keys) {
                let timeUnit = TimeUnit[key];
                if (this.value !== timeUnit) {
                    values.push(timeUnit);
                }
            }
            callback(values);
        });
        ...
        this.valuePanel.setLabelCallback((label: string) => {
            let request = new TimeUnitValueSetRequest(label);
