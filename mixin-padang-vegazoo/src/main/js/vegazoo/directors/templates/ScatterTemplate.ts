...
export default class ScatterTemplate implements DesignTemplate {

    public static MARK_TYPE = "circle";
    public static SCATTER = "Scatter";
    public static ENCODING = {
        x: { label: "X", field: "label", type: StandardType.QUANTITATIVE },
        y: { label: "Y", field: "count", type: StandardType.QUANTITATIVE },
    }

    public getName(): string {
        return ScatterTemplate.SCATTER;
    }
    ...
    public getEncodingRemapper(): EncodingRemapper {
        return new ScatterEncodingRemapper();
    }

    public getMarkType(): string {
        return ScatterTemplate.MARK_TYPE;
    }

    public getEncodingLabel(key: string): string {
        return ScatterTemplate.ENCODING[key].label;
    }

}

class ScatterEncodingRemapper extends ChannelEncodingRemapper {

    public remapFeature(oldMapping: { [name: string]: EObject }): { [name: string]: EObject } {
        let newMapping: { [name: string]: EObject } = {};
        this.mapField(oldMapping, "x", newMapping, [StandardType.QUANTITATIVE]);
        this.mapField(oldMapping, "y", newMapping, [StandardType.QUANTITATIVE]);
        return newMapping;
    }
...
factory.register(ScatterTemplate.MARK_TYPE, new ScatterTemplate());