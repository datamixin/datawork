/*
 * Copyright (c) 2020-2023 Datamixin.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>. */
import FunctionPlan from "padang/plan/FunctionPlan";
import ParameterPlan from "padang/plan/ParameterPlan";
import ParameterPlanUtils from "padang/plan/ParameterPlanUtils";
import FunctionPlanRegistry from "padang/plan/FunctionPlanRegistry";

import LabelEncoder from "padang/functions/encoder/LabelEncoder";

import ColumnFunction from "padang/functions/dataset/ColumnFunction";
import DatasetFunction from "padang/functions/dataset/DatasetFunction";

export default class EncodeColumn extends DatasetFunction {

	public static FUNCTION_NAME = "EncodeColumn";

	constructor(
		public dataset: string,
		public columns: string[],
		public encoder: any) {
		super(EncodeColumn.FUNCTION_NAME, dataset);
	}

	public static COLUMN_PLAN: ParameterPlan = ParameterPlanUtils.createTextPlan(
		"column",
		"Column",
		"Column name to encode",
		"",
		ColumnFunction.getDefaultColumnKeysAssignable()
	);

	public static ENCODER_PLAN: ParameterPlan = ParameterPlanUtils.createAnyPlan(
		"encoder",
		"Encoder",
		"Encoder used to encode columns",
		"=[" +
		LabelEncoder.FUNCTION_NAME +
		"]",
		LabelEncoder.FUNCTION_NAME + "()"
	);

	public static getPlan(): FunctionPlan {
		let plan = new FunctionPlan(
			EncodeColumn.FUNCTION_NAME,
			"Encode Columns",
			"mdi-table-column-width",
			"Encode list of columns by names"
		);
		let parameters = plan.getParameterList();
		parameters.add(DatasetFunction.DATASET_PLAN);
		parameters.add(EncodeColumn.COLUMN_PLAN);
		parameters.add(EncodeColumn.ENCODER_PLAN);
		return plan;
	}

}

let registry = FunctionPlanRegistry.getInstance();
registry.registerPlan(EncodeColumn.getPlan());