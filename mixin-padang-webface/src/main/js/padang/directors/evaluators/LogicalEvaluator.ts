...
export default class LogicalEvaluator extends ExpressionEvaluator {

    private value: boolean = true;

    public init(expression: XLogical): void {
        this.value = expression.toValue();
    }

    public evaluate(): any {
        return new VisageLogical(this.value);
    }
}

let factory = ExpressionEvaluatorFactory.getInstance();
factory.register(XLogical.XCLASSNAME, LogicalEvaluator);
