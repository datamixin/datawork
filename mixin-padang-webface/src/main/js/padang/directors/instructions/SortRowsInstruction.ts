...
export default class SortRowsInstruction extends Instruction {

    public createCaption(options: { [name: string]: XExpression }): string {
        let keys = options[SortRows.ORDERS_PLAN.getName()].toLiteral();
        return "Order by " + keys + "";
    }

}

let plan = SortRows.getPlan();
let registry = InstructionRegistry.getInstance();
registry.register(SortRows.FUNCTION_NAME, new SortRowsInstruction(plan));