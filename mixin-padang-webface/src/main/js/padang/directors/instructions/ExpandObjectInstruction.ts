...
export default class ExpandObjectInstruction extends Instruction {

	public createCaption(options: { [name: string]: XObject }): string {
		let name = options[ExpandObject.COLUMN_PLAN.getName()];
		return "ExpandObject column " + name;
	}

}

let plan = ExpandObject.getPlan();
let registry = InstructionRegistry.getInstance();
registry.register(ExpandObject.FUNCTION_NAME, new ExpandObjectInstruction(plan));