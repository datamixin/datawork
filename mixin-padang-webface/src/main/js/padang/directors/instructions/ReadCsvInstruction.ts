...
export default class ReadCsvInstruction extends Instruction {

	public createCaption(options: { [name: string]: XObject }): string {
		let name = options[ReadCsv.PATH_PLAN.getName()];
		return "Read CSV " + name;
	}

}

let plan = ReadCsv.getPlan();
let registry = InstructionRegistry.getInstance();
registry.register(ReadCsv.FUNCTION_NAME, new ReadCsvInstruction(plan));