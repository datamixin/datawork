...
export default class ExpandTableInstruction extends Instruction {

	public createCaption(options: { [name: string]: XObject }): string {
		let name = options[ExpandTable.COLUMN_PLAN.getName()];
		return "ExpandTable column " + name;
	}

}

let plan = ExpandTable.getPlan();
let registry = InstructionRegistry.getInstance();
registry.register(ExpandTable.FUNCTION_NAME, new ExpandTableInstruction(plan));