...
export default class FacetPropertyGetHandler extends BaseHandler {

    public handle(request: FacetPropertyGetRequest, callback: (format: string) => void): void {
        let keys = <string[]>request.getData(FacetPropertyGetRequest.KEYS);
        let properties = new ControllerProperties(this.controller, XFacet.FEATURE_PROPERTIES);
        let width = properties.getProperty(keys);
        callback(width);
    }
