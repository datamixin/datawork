...
export default class ExpandObjectDialog extends GuideDialog {
	...
	protected createControl(parent: Composite): void {
		...
		this.columnSupport = new NameListSupport(this.conductor, ExpandObject.COLUMN_PLAN);
		let listPlan = <ListPlan>ExpandObject.INCLUDES_PLAN.getAssignedPlan();
	...
	private createColumnPart(parent: Composite): void {
		...
		this.column = this.getText(ExpandObject.COLUMN_PLAN);
	...
	private createIncludesPart(parent: Composite): void {
		...
		this.includes = this.getList(ExpandObject.INCLUDES_PLAN);
...
factory.register(ExpandObject.FUNCTION_NAME, ExpandObjectDialog);