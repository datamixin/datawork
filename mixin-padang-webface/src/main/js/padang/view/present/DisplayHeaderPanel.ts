...
export default class DisplayHeaderPanel extends ConductorPanel {
    ...
    public createControl(parent: Composite, index?: number): void {
        ...
        element.addClass("padang-display-header-panel");
    ...
    public setMessage(message: string): void {
        this.briefPanel.setText(message);
    }

    public setRowColumn(rowCount: number, columnCount: number): void {
        view.grabVerticalExclusive(this.briefPanel);
        let text = "";
        if (rowCount !== -1) {
            text += rowCount + " rows";
