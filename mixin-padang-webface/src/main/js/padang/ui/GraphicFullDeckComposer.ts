 ...
 */
export default class GraphicFullDeckComposer extends BasePartViewer implements FullDeckPanel {

	private static ANATOMY_WIDTH = 320;
	private static EXPLAIN_HEIGHT = 360;
	...
	constructor(aspect: GraphicPremise) {
		super();
		this.graphic = <XGraphic>aspect.getModel();
		this.anatomyPartViewer.setParent(this);
		this.explainPartViewer.setParent(this);
		this.preparePreformPartViewer(aspect);
	...
	private preparePreformPartViewer(mapping: GraphicPremise): void {
		let registry = RendererRegistry.getInstance();
		let name = this.graphic.getRenderer();
		let renderer = registry.get(name);
		this.preformPartViewer = renderer.createPartViewer(mapping);
	...
	public createControl(parent: Composite): void {
		...
		element.addClass("graphic-composer");
	...
	private createAnatomyPartViewer(parent: Composite): void {
		...
		let layoutData = new GridData(GraphicFullDeckComposer.ANATOMY_WIDTH, true);
		control.setLayoutData(layoutData);
	}
	...
	private createExplainPartViewer(parent: Composite): void {
		...
		let layoutData = new GridData(GraphicFullDeckComposer.ANATOMY_WIDTH, GraphicFullDeckComposer.EXPLAIN_HEIGHT);
		control.setLayoutData(layoutData);
	}
