...
class ConditionalValidation(Validation):
    ...
    def validate(self, context: Context, hoster: Context) -> any:
        self._logical.validate(context, hoster)
        self._consequent.validate(context, hoster)
        self._alternate.validate(context, hoster)


factory: ValidationFactory = ValidationFactory.getInstance()
factory.register(EvaluateExpression.conditional.DESCRIPTOR.name, ConditionalValidation)
