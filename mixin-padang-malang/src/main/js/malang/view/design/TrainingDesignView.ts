...
export abstract class TrainingDesignView extends ConductorView {
	...
	public createControl(parent: Composite, index: number): void {
		...
		view.addClass(this.composite, "malang-training-design-view");
		view.setGridLayout(this.composite, 1, 0, 0, 0, TrainingDesignView.SPACING);
	...
	private createNamePart(parent: Composite): void {
		...
		view.addClass(this.namePart, "malang-training-design-name-part");
		view.setGridLayout(this.namePart, 2, 0, 0, 0, 0);
		view.setGridData(this.namePart, true, TrainingDesignView.LABEL_HEIGHT + 2)
	...
	private createLabel(parent: Composite): void {
		...
		view.css(label, "line-height", TrainingDesignView.LABEL_HEIGHT + "px");
		view.setGridData(label, TrainingDesignView.LABEL_WIDTH, TrainingDesignView.LABEL_HEIGHT);
	}

	private createNameDialogPanel(parent: Composite): void {
		this.namePanel = new LabelPopupPanel();
		this.namePanel.createControl(parent);
		view.css(this.namePanel, "line-height", TrainingDesignView.LABEL_HEIGHT + "px");
		...
		this.namePanel.setPopupActions((callback: (actions: BaseAction[]) => void) => {
			...
			for (let name of this.names.keys()) {
				...
				let action = new TrainingChangeAction(this.conductor, name, model);
				actions.push(action);
			}
			callback(actions);
		});
	...
	private createContainer(parent: Composite): void {
		...
		view.addClass(this.container, "malang-training-design-container");
		view.setGridLayout(this.container, 1, 0, 0, 0, TrainingDesignView.SPACING);
	...
	public adjustHeight(): number {
		...
		return height + TrainingDesignView.LABEL_HEIGHT + TrainingDesignView.SPACING + 2;
	}
...
export default TrainingDesignView;

class TrainingChangeAction extends BaseAction {
	...
	public run(): void {
		let request = new TrainingChangeRequest(this.model);
