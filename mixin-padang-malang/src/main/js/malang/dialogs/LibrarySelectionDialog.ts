...
export default class LibrarySelectionDialog extends TitleAreaDialog {
	...
	constructor(conductor: Conductor) {
		...
		this.setDialogSize(LibrarySelectionDialog.INIT_WIDTH, LibrarySelectionDialog.INIT_HEIGHT);
		this.setWindowTitle("Library Selection Dialog");
	...
	protected createControl(parent: Composite): void {
		...
		this.createFieldLabel(composite, "Library List", LibrarySelectionDialog.LIST_WIDTH);
		this.createFieldLabel(composite, "Library Detail", true);
	...
	private createFieldLabel(parent: Composite, text: string, width: number | boolean): void {
		...
		widgets.setGridData(label, width, LibrarySelectionDialog.ITEM_HEIGHT);
	}

	private createLabel(parent: Composite, text: string): Label {
		let label = new Label(parent);
		label.setText(text);
		widgets.css(label, "line-height", LibrarySelectionDialog.ITEM_HEIGHT + "px");
		return label;
	}
	...
	private createListViewerPart(parent: Composite): void {
		...
		this.listViewer.setContentProvider(new LibraryContentProvider(this.conductor));
		this.listViewer.setLabelProvider(new LibraryLabelProvider(this.conductor));

		let element = this.listViewer.getElement();
		element.css("border", "1px solid #D8D8D8");

		let layoutData = new GridData(LibrarySelectionDialog.LIST_WIDTH, true);
...
class LibraryContentProvider extends BaseProvider implements ContentProvider {
...
class LibraryLabelProvider extends BaseProvider implements TableLabelProvider {

	private columns = ["Library"];
