...
export default class ClassificationTaskPreload extends PlotPreload {

	constructor(name: string, presume: string, plan: PlotPlan, header?: boolean) {
		super(ClassificationAlgorithm.GROUP, name, presume, plan, header);
	}
	...
	public isApplicable(support: PreloadSupport, model: XModeler): boolean {
		...
		if (learning instanceof XSupervisedLearning) {
			...
			match = name === AlgorithmPlan.CLASSIFICATION;
		}
		if (match) {
...
let addPlot = (presume: string, plan: PlotPlan, header?: boolean) => {
	...
	let preload = new ClassificationTaskPreload(name, presume, plan, header);
...
let addMatrixPlot = (plan: PlotPlan) => {
	addPlot(malang.RESULT_ICON_MAP.MATRIX, plan);
}

addNumberPlot(AccuracyNumberPlot.getPlan(), false);
addMatrixPlot(ClassificationPerformancePlot.getPlan());
addMatrixPlot(ClassificationMatrixPlot.getPlan());
